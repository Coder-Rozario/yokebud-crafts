...
app.get('/api/category-sections', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.query('SELECT id, name, slug, created_at FROM category_sections ORDER BY id ASC');
    connection.release();
...
app.post('/api/category-sections', requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    connection = await pool.getConnection();
    const [result] = await connection.query(
      'INSERT INTO category_sections (name, slug) VALUES (?, ?)',
      [name, finalSlug]
    );

    connection.release();
    res.json({ success: true, message: 'Section created', section: { id: result.insertId, name, slug: finalSlug } });
  } catch (error) {
    if (connection) connection.release();
