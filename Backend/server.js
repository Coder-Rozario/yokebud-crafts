...
const hasAdminSession = (req) => Boolean(req.cookies && req.cookies.adminAuth === 'authenticated');
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const [discountRangeRows] = await connection.query(
      'SELECT * FROM quantity_discount_ranges ORDER BY product_id ASC, min_quantity ASC'
    );
    ...
    const discountRangeMap = new Map();
    for (const range of discountRangeRows) {
      const productId = Number(range.product_id);
      if (!discountRangeMap.has(productId)) {
        discountRangeMap.set(productId, []);
      }
      discountRangeMap.get(productId).push(range);
    }
    ...
    const parsedProducts = products.map(product => {
      ...
      return {
        ...
        discount_ranges: discountRangeMap.get(Number(product.id)) || []
      };
    });
...
const getWeeklyNewsletterCollections = async (connection) => {
  // Retrieve active discounted products (limit to top 10)
  ...
  // Retrieve products created in the last 7 days (limit to top 20)
