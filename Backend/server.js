...
async function ensureContentManagementSchema() {
  let connection;
  try {
    connection = await pool.getConnection();
    await connection.query(
      `CREATE TABLE IF NOT EXISTS content_management (
        ...
        content_type VARCHAR(50) DEFAULT 'text', -- 'text', 'html', 'textarea'
        content_value TEXT,
        description TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (id),
        UNIQUE KEY uniq_page_section (page_name, section_key)
      )`
    );
    ...
    const defaultContent = [
      // Home Page
      {
        ...
        section_key: 'home_hero_title',
        content_type: 'text',
        content_value: 'Craft Your Story',
        description: 'Hero section main title'
      },
      {
        page_name: 'home',
        section_name: 'Hero Section',
        section_key: 'home_hero_subtitle',
        content_type: 'textarea',
        content_value: 'Premium Laser Engraving & Custom Products',
        description: 'Hero section subtitle'
      },
      {
        page_name: 'home',
        section_name: 'Hero Section',
        section_key: 'home_hero_description',
        content_type: 'html',
        content_value: 'Your vision, our craft. Personalized gifts, custom apparel, and artisan creations made with love in Finland.',
        description: 'Hero section description text'
      },
      {
        page_name: 'home',
        section_name: 'Hero Section',
        section_key: 'home_hero_button_text',
        content_type: 'text',
        content_value: 'Explore Collection',
      ...
      },
      {
        ...
        content_type: 'html',
        content_value: 'Yokebud Crafts is your trusted partner for personalized creations in Finland.',
      ...
      // Business With Us Page
      {
        page_name: 'business',
        section_name: 'Hero Section',
        section_key: 'business_hero_title',
        content_type: 'text',
        content_value: 'Premium Business Solutions',
        description: 'Business page hero title'
      },
      {
        page_name: 'business',
        section_name: 'Hero Section',
        section_key: 'business_hero_subtitle',
        content_type: 'textarea',
        content_value: 'Custom Corporate Gifts, Branding & Signage',
    ...
    for (const item of defaultContent) {
      await connection.query(
        `INSERT IGNORE INTO content_management 
        (page_name, section_name, section_key, content_type, content_value, description) 
        VALUES (?, ?, ?, ?, ?, ?)`,
        [item.page_name, item.section_name, item.section_key, item.content_type, item.content_value, item.description]
      );
    }
    
    console.log('✅ Content Management schema checked/created');
  } catch (e) {
    console.warn('Content Management schema setup error:', e.message || e);
  } finally {
    if (connection) connection.release();
  }
}

ensureContentManagementSchema();
...
// Public: Get content by page name
app.get('/api/content/:page', async (req, res) => {
  ...
  try {
    ...
    connection = await pool.getConnection();
    const [rows] = await connection.query(
      'SELECT section_key, content_value, content_type FROM content_management WHERE page_name = ?',
      [page]
    );
    connection.release();
    
    // Convert to key-value object for easy frontend access
    const content = {};
    rows.forEach(row => {
    ...
    if (connection) connection.release();
    res.status(500).json({ success: false, message: 'Failed to fetch content: ' + error.message });
  }
});
...
app.get('/api/content', async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM content_management ORDER BY page_name, section_name');
    connection.release();
    res.json({ success: true, data: rows });
  } catch (error) {
    if (connection) connection.release();
    res.status(500).json({ success: false, message: 'Failed to fetch all content: ' + error.message });
  }
});

// Admin: Get all content for management
...
app.put('/api/admin/content/:id', requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    connection = await pool.getConnection();
    await connection.query(
      'UPDATE content_management SET content_value = ?, updated_at = NOW() WHERE id = ?',
      [content_value, id]
    );
    connection.release();
...
// Admin: Add new content item
app.post('/api/admin/content', requireAdminAuth, async (req, res) => {
  let connection;
  try {
    const { page_name, section_name, section_key, content_type, content_value, description } = req.body;
    if (!page_name || !section_name || !section_key) {
      return res.status(400).json({ success: false, message: 'Page name, section name, and section key are required' });
    }
    connection = await pool.getConnection();
    const [result] = await connection.query(
      'INSERT INTO content_management (page_name, section_name, section_key, content_type, content_value, description) VALUES (?, ?, ?, ?, ?, ?)',
      [page_name, section_name, section_key, content_type || 'text', content_value || '', description || '']
    );
    connection.release();
    res.json({ success: true, message: 'Content item added successfully', id: result.insertId });
  } catch (error) {
    if (connection) connection.release();
    res.status(500).json({ success: false, message: 'Failed to add content: ' + error.message });
  }
});
...
app.delete('/api/admin/content/:id', requireAdminAuth, async (req, res) => {
  ...
  try {
    const { id } = req.params;
    connection = await pool.getConnection();
    await connection.query('DELETE FROM content_management WHERE id = ?', [id]);
    connection.release();
    res.json({ success: true, message: 'Content item deleted successfully' });
  } catch (error) {
    if (connection) connection.release();
    res.status(500).json({ success: false, message: 'Failed to delete content: ' + error.message });
  }
});
